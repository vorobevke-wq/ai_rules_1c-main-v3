...
- Use the applicable verification toolset as concrete success criteria. For BSL / metadata changes: `syntaxcheck`, `check_1c_code`, `review_1c_code`, ITS standards lookup, impact analysis via `search_metadata` template operations or `rlm-tools-bsl` helpers (`find_references_to_object`, `find_code_usages`, `find_call_hierarchy`, etc.). For Markdown / rules / documentation: verify referenced paths, links, structure, and internal consistency.
