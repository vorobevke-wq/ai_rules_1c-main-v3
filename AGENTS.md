...
- Use the applicable verification toolset as concrete success criteria. For BSL / metadata changes: `analyze_file`, `check_1c_code`, ITS standards lookup, impact analysis via `search_metadata` template operations or `rlm-tools-bsl` helpers (`find_references_to_object`, `find_code_usages`, `find_call_hierarchy`, etc.). For Markdown / rules / documentation: verify referenced paths, links, structure, and internal consistency.
...
1. **Mandatory scope.** MCP calls are mandatory only for risk-bearing 1C work when a relevant server is exposed: BSL / metadata edits or review, metadata XML, forms, integrations, refactoring, performance, runtime errors, platform API checks, impact analysis, syntax / quality validation, **and OpenSpec spec authoring whenever the artifact references concrete 1C facts** (metadata names, attributes, tabular sections, public API signatures, БСП subsystems, platform-version behaviour, project conventions — see the spec-authoring path in `## Development Procedure → Triage` and `content/rules/sdd-integrations.md`). Generic Markdown / rules / documentation work that makes no such factual claims does not require 1C MCP calls; validate structure, links, paths, and internal consistency instead.
2. **Conditional external knowledge.** Use platform docs, locally installed БСП skills whose folder names contain `bsp-`, and ITS MCP tools when the task depends on versioned platform behavior, reusable БСП APIs, or standards compliance. Do not call them for generic prose cleanup or rule-file editing unless such a fact is actually needed.
3. **Verify before writing BSL / metadata / specs — scoped hard gate.** Use the minimum evidence set from `tooling-playbooks`: quick-fix BSL edits use only the directly relevant code / syntax context; full-cycle BSL changes use templates, existing project code, metadata, and platform / БСП / ITS docs when those sources affect correctness; metadata XML / form changes use examples plus metadata validation / compilation tools; **OpenSpec spec authoring** uses `memory.md` for prior project notes plus the MCP tools that confirm every 1C fact the spec references (Metacode graph / `rlm-tools-bsl` for object shape, platform / БСП / ITS docs for API and standards) — see `content/rules/sdd-integrations.md`. In the final answer for any non-trivial BSL / metadata change **or non-trivial OpenSpec spec authoring (proposal / design / tasks / delta specs that reference real 1C objects, APIs, or БСП subsystems)**, list the context sources actually used and briefly state why any normally relevant source was skipped. Skipping a relevant source silently counts as a defect. When `bsl-language-server` is exposed, use `type_at_position` before writing or changing a call whose receiver type / available members are uncertain, use `type_info` when a known type name needs exact members or signatures, and use `hover` for review or explanation of a symbol at a known position.
4. **No blind chaining.** Every MCP call must close a concrete context gap. Follow the fallback order from `mcp-1c-tools`; do not duplicate calls or continue the chain after you already have enough evidence. If `1c-mcp-metacode` returns empty / non-actionable results twice on substantially different queries for the same target, fall back to `rlm-tools-bsl` (`rlm_start` → `rlm_help` when needed → batched `rlm_execute`) instead of further Metacode attempts. Before using `Grep` / `rg` for 1C project-source search, first exhaust the project-index search path from `mcp-1c-tools`, including the documented `git_search` / narrowed helper retry where applicable, and state why those attempts were insufficient.
5. **Validate changed 1C code.** After BSL / metadata edits: `analyze_file` → `check_1c_code`, within the verification budget defined in section B below. Use `check_1c_code` with `check_type="review"` when style / ITS compliance needs a separate pass. For metadata XML use schema / XML validation and prefer `1c-metadata-manage` for non-trivial changes.
6. **ITS documents.** Always follow `search_its` with `fetch_its` for every returned document ID you rely on.

### B. Limits and non-determinism

1. **Verification budget — 1 call per validator by default; up to 3 only if the previous run returned a substantive defect.** Applies separately to `analyze_file` and `check_1c_code` when validating BSL / metadata changes. A **cycle** = one logical edit of one module; every new edit starts a new cycle.
   ...
   - **Pure metadata-XML changes (no BSL touched)** — `check_1c_code` is usually irrelevant; skip it unless the metadata change embeds BSL (object / manager module, form module, fill-check expression, predefined-item population). Use the relevant `1c-metadata-manage` validator once instead (`form-validate`, `meta-validate`, `role-validate`, `skd-validate`, `mxl-validate`, etc.). `analyze_file` is still run on any BSL module touched, even indirectly (e.g. a form module regenerated by the metadata skill).
   - Markdown / rules / documentation edits use the docs-fix path checks instead.
2. **AI-based MCP tools are non-deterministic.** `ask_1c_ai`, `modify_1c_code`, and natural-language Metacode queries that require LLM translation produce drafts, not authority. Re-validate output via deterministic template queries and, for code changes, via `analyze_file` + `check_1c_code` before delivery (subject to the budget above).
...
2. **No-change repeats are forbidden.** Do not repeat the same tool request against the same unchanged state when the previous result is still available: same search, same MCP query, same validator input, or same file contents. A repeat is allowed when parameters change substantially (different query, different object, different depth), state changed (file edit, generated output, new checkout, resumed session, user edited the file), or freshness matters before a destructive action / final verification. Do not re-run `check_1c_code` if the code has not changed since the last run.
3. **Tune each query to the tool's schema.** Parameter-rich tools (`search_code`, `search_metadata`, `search_metadata_by_description`, `rlm_start`, `rlm_help`, `rlm_execute`, `rlm_index`, `execute_query`, `execute_code`, `get_metadata`, `get_event_log`) — defaults usually suboptimal. Before such a call, consult `mcp-1c-tools/docs/<server>.md` (the environment descriptor wins on conflict if exposed) and tune the relevant parameters: `query`, `path` / `project`, `effort`, `topic`, `helpers`, `category`, `section`, index action parameters, Toolkit filters / limits / `execution_context`, plus the helper arguments used inside `rlm_execute` (`search` scope, `git_search` filters, `find_call_hierarchy` depth / `module_hint`, exact 1C names, dotted names, GUIDs for `find_by_guid`, JSON templates for `search_metadata`). Narrow scope with project / category filters when the live schema exposes them. If the first call returns nothing, reformulate (broaden / narrow, switch mode, choose a different RLM helper, raise result limits) before falling back to another tool. **This rule is about call quality — it does not relax the obligation to make the call.**
4. **Prefer structural tools over manual grep.** In `rlm-tools-bsl`, prefer helpers such as `search_methods`, `find_module`, `extract_procedures`, `read_procedure`, `find_call_hierarchy`, `get_object_full_structure`, and `parse_form` for code / metadata navigation before falling back to substring search.
5. **Do not invent parameter names — sync against `docs/<server>.md` before the call.** Use the exact argument names from `mcp-1c-tools/docs/<server>.md` or the live tool schema; never substitute a "natural-sounding" alias. Mandatory verification: before the first call in the session to any MCP tool whose parameter names are not obvious from a short routine call (in particular every tool listed under *Parameter-rich tools — read the doc first* in `mcp-1c-tools/SKILL.md`, every RLM top-level tool / unfamiliar helper on `rlm-tools-bsl`, and every tool you have not called in this session yet), open the corresponding `docs/<server>.md` and confirm the exact parameter names and value formats. Re-confirm whenever you switch to a different tool on the same server. Skipping this check and calling with a guessed parameter name is a defect.
   - **`1c-mcp-metacode`** — documented top-level tools are `search_metadata`, `search_metadata_by_description`, and `search_code`; pass the request text or JSON template as **`query`** unless the live schema says otherwise. Retired graph-server top-level tools are not available; express those needs through `search_metadata` template operations such as `object_structure`, `find_objects_using_object`, `find_usages_of_object`, `find_documents_making_movements_into_register`, `resolve_qn`, `list_callers_of_routine`, `list_callees_of_routine`, and `call_graph_subtree`.
   - **`rlm-tools-bsl`** — top-level MCP tools use session arguments, not object-scoped aliases: `rlm_start(query, path|project, ...)`, `rlm_help(topic|helpers|category|section, ...)`, `rlm_execute(session_id, code, ...)`, `rlm_end(session_id)`, `rlm_projects(action, ...)`, `rlm_index(action, ...)`. 1C-specific operations are Python helpers inside `rlm_execute`; confirm helper signatures through `rlm_start.available_functions` and `rlm_help(helpers=[...])` before using unfamiliar helpers.
   - **`bsl-language-server`** — `analyze_file` is file based: pass `file` with a `.bsl` / `.os` path that is absolute or relative to the server working directory, never raw BSL text. For `hover` / `type_at_position` / `definition` / `find_references`, pass zero-based `line` and `character`; for `type_info`, pass `typeName`, `fileType`, `root`, and optional `language`.
   - **`1c-mcp-toolkit`** — live-IB calls are parameter-rich and may mutate the connected infobase if misused: `execute_query(query, params?, limit?, include_schema?)`, `execute_code(code, execution_context="server"|"client")`, `get_metadata(filter?, meta_type?, name_mask?, sections?, extension_name?, attribute_mask?)`, `get_event_log(start_date?, end_date?, levels?, metadata_type?, user?, application?, comment_contains?, limit?, same_second_offset?)`. Use the parse-only `execute_code` wrapper with `Новый Запрос(<query>).НайтиПараметры()` when only query validation is needed.
   - For RLM helpers, pass the exact helper parameter names shown by `rlm_help`; do not invent aliases. Use 1C dotted qualified names where the helper expects object references (`Справочник.Контрагенты`, `Документ.РеализацияТоваровУслуг`) and module/path hints where the helper expects paths (`module_hint`, `path`).
...
The project has one long-term memory store: `memory.md` at the project root. Do not use `1c-templates-mcp` for project memory; that server is only a code-template library.
...
- **Non-eligible facts.** User corrections, temporary decisions, subsystem-scoped quirks, and task-local notes do not go into long-term memory unless they meet the criteria above. Keep them in the active artifact (`openspec/`, handoff, issue, PR, or final summary) instead of silently creating a durable project rule.
- **No secrets / PII.** Never write credentials, tokens, personal data, full connection strings, or customer-sensitive payloads to `memory.md`.
