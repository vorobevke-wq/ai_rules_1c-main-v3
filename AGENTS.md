...
- Use the applicable verification toolset as concrete success criteria. For BSL / metadata changes: `diagnostics`, `check_1c_code`, `review_1c_code`, ITS standards lookup, impact analysis via `search_metadata` template operations or `rlm-tools-bsl` helpers (`find_references_to_object`, `find_code_usages`, `find_call_hierarchy`, etc.). For Markdown / rules / documentation: verify referenced paths, links, structure, and internal consistency.
...
- **Highly desirable for LSP MCP connectivity** (`LSP_PROJECT_ID`) — needed only to render the `x-project-id` header for `1c-lsp-diagnostics`. Empty = installer leaves `{LSP_PROJECT_ID}` in MCP config and warns; ask only when configuring or troubleshooting `1c-lsp-mcp-skill`.
...
5. **Validate changed 1C code.** After BSL / metadata edits: `diagnostics` → `check_1c_code` → `review_1c_code`, within the verification budget defined in section B below. For metadata XML use schema / XML validation and prefer `1c-metadata-manage` for non-trivial changes.
...
1. **Verification budget — 1 call per validator by default; up to 3 only if the previous run returned a substantive defect.** Applies separately to `diagnostics`, `check_1c_code`, and `review_1c_code` when validating BSL / metadata changes. A **cycle** = one logical edit of one module; every new edit starts a new cycle.
...
2. **AI-based MCP tools are non-deterministic.** `ask_1c_ai`, `rewrite_1c_code`, `modify_1c_code`, and natural-language Metacode queries that require LLM translation produce drafts, not authority. Re-validate output via deterministic template queries and, for code changes, via `diagnostics` + `check_1c_code` + `review_1c_code` before delivery (subject to the budget above).
...
- **verification-checklist** — unified "done" gate for any non-trivial change: ordered hard gates (`diagnostics` → `check_1c_code` → `review_1c_code` → impact analysis → metadata XML validation) plus soft gates (debug reproduction, plan adherence, user-explicit code review). Load before declaring any non-trivial change finished. File: `content/rules/verification-checklist.md`.
