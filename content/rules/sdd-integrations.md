...
- **quick-spec** — change touches **one** existing metadata object **plus**, optionally, 1-3 independent isolated additions (a new constant, a new data processor / settings form, a new independent information register with no module). No new documents / accumulation or accounting registers / roles / event subscriptions / scheduled jobs. No changes to existing transactional paths, RLS conditions, posting code, or public common-module signatures. Naming of new objects is the only architecturally novel decision.
  *Evidence minimum:* targeted attribute check via `search_metadata` JSON template (see check 2 below) **plus** installed `bsp-*` skill documentation and project-source confirmation if the spec relies on a БСП subsystem **plus** `memory.md` review only if existing entries overlap with the change keywords. `Context sources` block — one line.
...
1. **Project memory — `memory.md`.** Review relevant entries when the change keywords overlap with durable project rules: existing object names (`НачислениеЗарплаты`, `ПродажиТовары`), known subsystems (`Документооборот`, `ИнтернетПоддержка`), recurring critical constraints, or prior architectural decisions promoted to long-term memory. For genuinely greenfield topics — a domain the project has never touched — `memory.md` review is optional; a single short note in `Context sources` ("`memory.md` skipped: greenfield topic") is enough. Catches existing project-wide rules and settled architectural constraints.
2. **Metadata facts — prefer targeted queries over full dossiers.** Choose the narrowest method that closes the gap:
   ...
   - **Fallback chain on empty / non-actionable results** — `rlm-tools-bsl` session helpers → `git_search` / narrowed RLM retry → `Grep` (per `AGENTS.md → Tooling & Standards → A.4`).
   Do not invent attribute names from analogous documents or from memory.
3. **Platform APIs — `1c-syntax` (`search_syntax`, `get_function_info`, `validate_syntax`) and ITS (`search_its` → `fetch_its`).** Every platform type, method, or behaviour the spec relies on (`HTTPСоединение`, `ЗащищённоеСоединениеOpenSSL`, `ЗаписатьJSON` / `ПрочитатьJSON`, `ДлительныеОперации`, async / `Ждать`, role permissions, etc.) — verify the exact name, signature, and version availability against the project's `CompatibilityMode` **when the spec is normative about that API**. Memory-written API signatures are not evidence. Skip for hrestomatic APIs whose shape is fixed across all supported versions and where the spec does not pin a specific signature.
4. **БСП / SSL — installed `bsp-*` skills + project source.** When the spec mentions integrating with a БСП subsystem (`Администрирование`, `ИнтернетПоддержкаПользователей`, `ПолучениеФайловИзИнтернета`, `ЦифроваяПодпись`, `ДлительныеОперации`, `ОчередьЗаданий`, `БезопасноеХранилище`, `ЗащитаПерсональныхДанных`, …), load the relevant locally installed skill whose folder name contains `bsp-`, then confirm the subsystem actually exists in this project's БСП version, its real name in this configuration, and which public API to call through `openspec/project.md`, Metacode, or `rlm-tools-bsl` project-source helpers. Verify the БСП hook (`ПриОпределенииПодсистемСКоторымиВозможнаИнтеграция`, `ПриДобавленииЭлементовФормы`, etc.) exists in this БСП version. **Required without exception when the change introduces storage of secrets, tokens, or API keys** (confirm `БезопасноеХранилище` shape) **or touches personal data** (confirm `ЗащитаПерсональныхДанных` hooks).
5. **Project source patterns — `search_code` / `rlm-tools-bsl` helpers.** When the spec proposes a new module, function, or pattern, check whether the project already has a similar one to align naming, signature, and placement. Use RLM helpers such as `search`, `search_methods`, `find_module`, `extract_procedures`, and `git_search`. Skip when the new code has no analog in the project (genuinely first-of-its-kind).
...
- **TODO / "to be clarified" / "уточнить" for a fact one tool or skill lookup closes.** If you can answer it now via `search_metadata`, `get_function_info`, installed `bsp-*` skills, or project-source search, do it now. A TODO is allowed only for facts that genuinely depend on a human decision (business rule, naming preference, priority).
...
- **If the decision depends on a 1C fact that one tool or skill lookup could close — make the lookup, do not ask.** The user is not a substitute for `search_metadata`, installed `bsp-*` skills, or project-source search.
...
|----------|-------|--------|
| **1c-explorer** | `specs/`, current codebase, Metacode / `rlm-tools-bsl` indexes | read-only findings for `proposal.md`, `design.md`, or `tasks.md` authors; no artifact writes |
