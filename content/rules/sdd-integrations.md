...
- **TODO / "to be clarified" / "уточнить" for a fact one MCP call closes.** If you can answer it now via `recall` / `search_metadata` / `get_function_info` / `ssl_search`, do it now. A TODO is allowed only for facts that genuinely depend on a human decision (business rule, naming preference, priority).
