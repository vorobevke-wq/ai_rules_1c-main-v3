...
- `tooling-playbooks.md → Form Analysis and Generation` — full ordered list of calls (`rlm-tools-bsl` `parse_form` / `search_objects` / `get_object_full_structure` → write/modify XML → `form-validate` → compile via the `1c-metadata-manage` skill).
...
- **Form XML validation is mandatory** after any XML edit — use `form-validate` from `1c-metadata-manage`. A form that parses in your editor is not a form that loads in Designer.
