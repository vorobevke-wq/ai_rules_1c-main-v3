...
|---|---|---|
| **Quick-fix BSL** (single procedure, no metadata / transaction / public API impact) | Read the target module / procedure and any directly referenced helper needed to understand the bug | `analyze_file` on the touched `.bsl` file |
| **Full-cycle BSL** | `templatesearch` when a reusable pattern may exist; `search_code` / RLM helpers (`search`, `search_methods`, `find_definition`, `git_search`, `safe_grep`) for local patterns; `search_metadata` (`object_structure` / focused templates) or RLM helpers (`get_object_full_structure`, `find_attributes`) when metadata shape affects the code; `type_at_position` / `type_info` when exact receiver types or available members affect the implementation; platform / БСП / ITS docs only when versioned API or standard behaviour matters | `analyze_file` → `check_1c_code`; impact analysis when public surface or metadata usage changed |
...
| **Integrations / platform APIs** | Existing integrations, templates, relevant БСП APIs, platform docs for exact API names / version availability, security requirements | `analyze_file` → `check_1c_code`; ITS check when relying on an ITS standard |
...
8. **type_at_position / type_info** (`bsl-language-server`, if available) — before writing a call where the receiver type or available members are uncertain.
9. **get_function_info** — verify built-in functions by exact name; **search_syntax / suggest_completion** — find candidates by name or prefix.
10. **Installed `bsp-*` skills** — find reusable БСП functions and patterns; confirm project-specific availability through source search when needed.
11. **analyze_file** — verify touched `.bsl` files after writing.
12. **check_1c_code** — find syntax, logic, performance, style, and ITS-compliance defects.
13. **execute_code** query parse wrapper (`1c-mcp-toolkit`, if available) — when the change introduces a new / non-trivial query string (module code, DCS data set, dynamic list), parse-check it against the live IB by running `Новый Запрос(<query>).НайтиПараметры()` inside a read-only `execute_code` fragment before delivery. Especially important after non-deterministic AI generation (`modify_1c_code` / `ask_1c_ai`). Use `execute_query` with an intentionally empty result only when the query can be safely rewritten to return no rows.
...
5. **hover** (`bsl-language-server`, if available) — explain or review a concrete symbol at a known file position.
6. **get_function_info** — verify method/property existence; **search_syntax** — search by name fragment.
7. **check_1c_code** — bugs, performance issues, style, and ITS compliance.
8. **search_its** → **fetch_its** — cross-check against ITS standards.
9. **templatesearch** — architectural templates.
10. **ask_1c_ai** — architectural questions to 1С:Напарник (treat as a hint, not authority).
...
2. **analyze_file** — syntax and analyzer errors.
...
10. **type_at_position / hover** (`bsl-language-server`, if available) — inspect the failing expression or symbol when a precise file position is known.
11. **execute_code** query parse wrapper (`1c-mcp-toolkit`, if available) — when the suspect path is a query string, parse-check it through `Новый Запрос(<query>).НайтиПараметры()` before deeper investigation.
12. **execute_query** (`1c-mcp-toolkit`, if available) — read-only query against the live IB to confirm a data-state hypothesis without changing production code.
13. **execute_code** (`1c-mcp-toolkit`, if available) — run a small read-only BSL fragment in the live IB to verify a platform-version-specific behaviour. Default to read-only; **never** wrap a mutation without explicit user consent (see `docs/1c-mcp-toolkit.md → Safety and discipline`).
...
6. **modify_1c_code** — targeted AI optimization only with an explicit instruction; re-validate with `analyze_file` and `check_1c_code`.
...
6. **type_info** (`bsl-language-server`, if available) — exact members/signatures for a known platform type when `1c-syntax` is not enough or the type is configuration-specific.
7. **1c-metadata-manage / XML validation tooling** — when the contract is XML with a known XSD.
8. **search_its** → **fetch_its** — ITS articles on long-running operations, secure password storage, asynchronous external components.
9. **rlm-tools-bsl** (`search_methods`, `find_module`, `extract_procedures`) — locate or extend the integration common module (typically `*HTTPClient`, `*Integration`, `*Exchange`).
10. After implementation: **analyze_file** → **check_1c_code**.
...
4. **hover** (`bsl-language-server`, if available) — explain a concrete symbol at a known position.
5. **get_function_info** — syntax-reference information by exact name; **search_syntax** — search by name fragment.
6. **search_1c_documentation** — existing help articles.
7. **search_its** → **fetch_its** — methodological ITS articles.
8. **search_1c_documentation** — version-specific platform documentation.
