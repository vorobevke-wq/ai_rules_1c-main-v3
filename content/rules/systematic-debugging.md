...
**Goal:** replace ad-hoc trial-and-error with a structured root-cause loop. Skipping a phase is a defect, the same way as skipping `analyze_file` after editing code.
...
- **`get_event_log`** (`1c-mcp-toolkit`, when the server is exposed in the session) — pull relevant error records from `ЖурналРегистрации` of the live IB without leaving the agent: timestamp, event, affected metadata object, data presentation, full description. Start with `levels=["Error"]`, a recent `start_date`, and `limit=1` immediately after the user's repro attempt; add `metadata_type`, `user`, `application`, or `comment_contains` when the log is noisy. Wider filters and pagination are supported by the same tool.
...
- **`execute_code` query parse wrapper** → **`execute_query`** (`1c-mcp-toolkit`, when the server is exposed) — parse-check through `Новый Запрос(<query>).НайтиПараметры()`, then execute a falsifying query against the live IB without leaving the agent. Always read-only. Cheap evidence for a data-state hypothesis ("does this register really have a non-zero balance for this dimension set", "does this attribute really equal X for this reference").
- **`execute_code`** (`1c-mcp-toolkit`, when exposed) — run a small **read-only** BSL fragment to verify a platform-version-specific or metadata-state hypothesis (type checks, `ЗначениеЗаполнено`, `Метаданные.НайтиПоПолномуИмени`, `ПолучитьФункциональнуюОпцию`). Default to read-only; **never** wrap `Записать()` / `Удалить()` / `НачалоТранзакции` / DML in `execute_code` without explicit user consent and a rollback plan — and only on a copy IB. See `content/skills/mcp-1c-tools/docs/1c-mcp-toolkit.md → Safety and discipline` for full rules.
...
- Verification chain runs cleanly: `analyze_file` → `check_1c_code` → Metacode impact templates or fallback dependency analysis against the touched objects (see `verification-checklist.md`).
