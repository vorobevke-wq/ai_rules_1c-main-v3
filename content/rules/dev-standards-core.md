...
| `{LSP_PROJECT_ID}` | Project id configured in `lsp-skill-server`; rendered as the `x-project-id` MCP header for `1c-lsp-diagnostics` | Highly desirable for `1c-lsp-mcp-skill` connectivity | Empty = installer keeps `{LSP_PROJECT_ID}` in MCP config and warns. Ask only when configuring or troubleshooting `1c-lsp-mcp-skill`; unrelated code / docs work proceeds |
