...
- `rlm-tools-bsl`: `rlm_start`, `rlm_help`, `rlm_execute`, `rlm_projects`, `rlm_index`; inside `rlm_execute`, tune helper choice and arguments (`search`, `search_objects`, `search_methods`, `find_definition`, `get_module_outline`, `git_search(..., exclude_path=...)`, `safe_grep(...)`, `get_object_full_structure`, `parse_form`, `find_call_hierarchy(..., include_triggers=...)`, `find_path`, `find_data_path`, `find_references_to_object`, etc.) and read coverage/provenance fields such as `source`, `owner`, `extensions_included`, `total_exact`, `index_coverage`, `truncated`, and `has_more` when present.
...
| **rlm-tools-bsl** | Intent-first 1C BSL repository search and navigation through an RLM sandbox: project registry, optional SQLite index, metadata/code/form/usages/call-graph/data-path helpers, extension override analysis, and coverage/provenance signals | [`docs/rlm-tools-bsl.md`](docs/rlm-tools-bsl.md) |
