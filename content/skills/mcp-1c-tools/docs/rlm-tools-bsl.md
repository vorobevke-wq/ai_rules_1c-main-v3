...
Last synced against upstream `v1.32.0` / `master` on 2026-08-07.
...
| Forms | `parse_form(object_name, form_name="", handler="")`; use `find_module(name)` + `read_procedure` for form-module handler bodies. Form attribute `types` is a comma-separated string, not a list; split it only when list semantics are needed. |
...
| Extension overrides | `detect_extensions()` (`nearby_extensions[].overrides_count`), `find_ext_overrides(extension_path, object_name)`, and `get_overrides(object_name, method_name)`. `get_overrides` returns the first 200 rows plus authoritative aggregate fields (`by_annotation`, top objects/extensions, unique counts) when `partial=false`; never recompute totals from the capped `overrides` slice. |
...
- `find_functional_options(..., limit=N)` paginates XML and code buckets independently; pass `limit` by name because the second positional argument is `include_code`. For an empty-object overview, `partial=true` with `_meta.reason="code_scan_budget"` means totals cover all XML plus only the reported code scan slice.
...
- `get_overrides()` returns a deterministic first 200-row slice plus `total`, `truncated`, `partial`, source, annotation/object/extension aggregates, and unique counts. Use server-provided aggregates instead of summarizing the capped slice; when `partial=true`, totals are lower bounds and `_meta.failed_extension_roots` identifies unread roots. Filter by object/method or use `find_ext_overrides()` for detailed extension rows. `get_overrides()` and `find_ext_overrides()` now normalize extension row aliases such as `module_path` / `ext_module_path`, `line` / `ext_line`, and `module_type`; prefer the normalized keys but do not assume missing line data means an index defect.
- `parse_form(...).attributes[].types` is a comma-separated string (`"Type1, Type2"`), not a list; use `types.split(", ") if types else []` when iterating types. In `find_references_to_object`, `line` is a best-effort anchor near the start of the matching XML block, not necessarily the exact type tag.
