...
Last synced against upstream `v1.26.0` / `master` on 2026-06-28.
...
| Index status | `rlm_start.index.index_status` for startup state; `get_index_info()` inside the session when an explicit refresh/diagnostic is needed; `rlm_index(action="info", project=...)` outside the session. |
...
- `get_object_profile` is the preferred first hop for a compact object overview. `get_object_modules` is the code-side aggregate for modules and outlines; combine it with `get_object_full_structure` for metadata-side details.
- `rlm_execute` may return `efficiency_hints` such as batch, reuse-variable, or read-files guidance. Follow them in later calls; they are response metadata, not helper stdout.
