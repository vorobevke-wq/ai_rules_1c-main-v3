...
- **Do not pipe AI output blindly.** Output of `rewrite_1c_code`, `modify_1c_code`, `ask_1c_ai` is non-deterministic. Validate query strings with `validatequery` first; validate changed BSL files with `diagnostics` first; only then consider `vcexecutequery` / `vcexecutecode` against the live IB.
