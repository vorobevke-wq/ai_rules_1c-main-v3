...
- **Do not pipe AI output blindly.** Output from `ask_1c_ai` or `modify_1c_code` is non-deterministic. Validate BSL through `analyze_file` / `check_1c_code`; validate query strings through the parse-only `execute_code` wrapper before executing them.
