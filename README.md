...
Сервер требует заголовок `x-project-id` с ID проекта из `lsp-skill-server`. Этот заголовок настраивается вручную в активном MCP-конфиге проекта (`mcp.json`, `.cursor/mcp.json`, `.codex/config.toml`, `opencode.json` или аналогичном файле клиента); установщик 1c-rules его не генерирует.
