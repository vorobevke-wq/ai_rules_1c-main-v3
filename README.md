...
- **Память проекта** — `memory.md`: строгий долговременный слой для глобальных критичных правил проекта. Это не общий блокнот; критерии записи описаны в `AGENTS.md → Project memory`.
...
- `query-design.md` — единая точка входа для запросов: выбирает правила написания или оптимизации; для оптимизации и новых пакетных запросов подключает обязательный `Mandatory Optimization Checklist`.
...
- **Запросы** — единая точка входа `query-design.md`: написание новых (`query-writing.md`) и оптимизация (`query-optimization.md`); для любой задачи «оптимизируй запрос» обязателен `Mandatory Optimization Checklist`.
...
- **`handoff`** — сжимает текущий разговор в самодостаточный handoff-документ для следующей сессии (новый чат, другая машина, другой ИИ-клиент). Не дублирует durable-артефакты (`openspec/`, `memory.md`, коммиты), а ссылается на них. Дефолтный путь — `handoffs/handoff-<timestamp>.md` в корне проекта. Адаптировано из `mattpocock/skills`.
...
Fallback к графовому MCP и компактный способ исследовать большую выгрузку 1С без заливки сырых файлов в контекст. Сервер сейчас описан intent-first: при задачах поиска модулей, методов, ссылок, графа вызовов, форм и XML агент должен тянуться к `rlm-tools-bsl` до сырого `grep`. Сессия открывается через `rlm_start`, подробные рецепты и сигнатуры доступны через `rlm_help` в slim-режиме, хелперы вызываются Python-кодом в `rlm_execute`, завершение — `rlm_end`. Глубина `rlm_start` по умолчанию выбирается сервером (`effort="auto"`), а `rlm_start.index` уже содержит статус индекса, `builder_version` и discovery-счётчики. Полнота ответов читается по `source`, `owner`, `extensions_included`, `total_exact`, `index_coverage`, `truncated` / `has_more`; для неоднозначных выводов есть `rlm_help(section="coverage")`. На конфигурациях с большим числом расширений `extension_context.nearby_extensions` может быть усечён до top-N; полный список нужно добирать через `detect_extensions()` / `get_overrides()`. Для часто используемых исходников есть реестр проектов (`rlm_projects`) и опциональный SQLite-индекс (`rlm_index`); мутирующие операции реестра/индекса требуют пароль проекта, если сервер вернул `approval_required`.

- **Метаданные и граф данных**: `get_object_profile`, `search_objects`, `find_by_type`, `find_attributes`, `get_object_full_structure`, `parse_object_xml`, `find_references_to_object`, `find_code_usages`, `find_data_path`, `find_defined_types`.
- **Код и структура модулей**: `get_object_modules`, `search`, `search_methods`, `find_definition`, `find_module`, `get_module_outline`, `extract_procedures`, `find_exports`, `read_procedure` (включая batch-списки), `safe_grep`, `code_metrics`.
- **Полнотекстовый поиск**: `git_search(pattern, path="", file_types="", regex=False, ignore_case=False, mode="lines", max_results=..., exclude_path="")`; `git_search` и `safe_grep` возвращают словарь с `results`, `returned`, `truncated`, а `exclude_path` отсекает шумовые зоны вроде `Forms` / `Templates` на любой глубине.
...
- **Формы, интеграции и расширения**: `parse_form` с типами реквизитов формы как списком, `find_http_services`, `find_web_services`, `find_xdto_packages`, `find_exchange_plan_content`, `detect_extensions`, `get_overrides` / `find_ext_overrides` с `total` / `returned` / `has_more` / `truncated`.
...
- `templatesearch` — гибридный поиск по библиотеке шаблонов; вызывается **до** написания кода, когда может существовать готовый паттерн.
- `list_templates` — список шаблонов с пагинацией.
- `get_template` — получение полного шаблона с кодом по ID.

Мутирующие инструменты библиотеки шаблонов в правилах не описываются и не используются. Проектная память не хранится в `1c-templates-mcp`; для неё остаётся только `memory.md`.

### БСП-скиллы

Для справки по БСП используйте локальные скиллы, папки которых содержат `bsp-`; после установки правил в проект их можно вручную скопировать в skills-каталог активного инструмента (`.codex/skills/`, `.kilo/skills/`, `.ai-agent/skills/` и т.п.).
