...
4. **Apply frontmatter operations and rewrite installed paths.** For sections that have `frontmatter.keep` / `drop` / `rename` / `addIf` / `toolsToPermission`:
   - During the frontmatter pass, read **only** the YAML frontmatter block (between the leading `---` markers — typically the first 5–20 lines). Do not read the body yet.
   - Rewrite the frontmatter according to the adapter ops and write it back; this pass leaves the body untouched.
   ...
   - For sections with `mode: verbatim` (skills) — skip the frontmatter step entirely. The Markdown path rewrite below still applies; scripts, images, templates, and other non-Markdown files remain byte-for-byte copies.
   ...
   - After rendering each rule, agent, or command, rewrite every source path to the current tool's own adapter layout: `content/rules/<name>.md`, `content/agents/<name>.md`, and `content/commands/<name>.md` use the target directory and extension from the matching `copyTo`; `content/skills/<rest>` uses the target skills directory and preserves the remaining subpath; `content/mcp-servers.json` becomes that tool's `mcp.target`. Apply the same rewrite to every copied Markdown file under skills and `content/openspec-bundle/<tool>/`. This rewrite is per-tool, not globally canonical: a Codex agent must point to `.codex/...` even when Cursor is installed in the same project. Wildcard examples such as `content/rules/*.md` must also receive the target extension (`.mdc` for Cursor). The source files remain unchanged.

5. **Render the MCP config** from `content/mcp-servers.json` according to the adapter's `mcp.schema` (mcpServers JSON dictionary, OpenCode `mcp[id]` schema, or Codex TOML `[mcp_servers.<id>]`). Preserve supported per-server `headers` from the catalog. `1c-lsp-diagnostics` requires an `x-project-id` header from `lsp-skill-server`; the installer renders it from `content/mcp-servers.json` as the `<project-id>` placeholder, and the user must replace that placeholder with the real project id after `lsp-skill-server` is configured. **Codex local MCP entries** may also render `cwd`, `env`, `startup_timeout_sec`, `tool_timeout_sec`, `required`, `enabled_tools`, and `default_tools_approval_mode` when those fields are present in the catalog. **Claude Code only:** remote HTTP entries must emit `{ "type": "http", "url": ... }` (plus `headers` when present) and must not inherit Cursor-only `connection_id` / `description` keys; without explicit `type`, current Claude Code silently does not load the server. **OpenCode only:** normalize each server key to start with a letter before writing it under `mcp` — leading `1c`/`1C` → `onec` (so `1c-lsp-diagnostics` → `onec-lsp-diagnostics`, `1c-syntax` → `onec-syntax`), any other non-letter-leading id gets an `mcp-` prefix. OpenCode names MCP tools `<server-key>_<tool>` and providers like Moonshot/Kimi reject function names that do not start with a letter (`^[a-zA-Z_]…`), which otherwise breaks the whole request with *"function name is invalid, must start with a letter"*. Canonical ids in `content/mcp-servers.json` and the docs stay `1c-…`; only the OpenCode-rendered key changes. The other adapters keep the verbatim id (Cursor/Claude prefix tool names with `mcp_`/`mcp__`, so a digit-leading key is already safe there). **Target file & merge:** write each tool's MCP into the path declared by `mcp.target` — for OpenCode this is `opencode.json` at the **project root** (NOT `.opencode/opencode.json`, which OpenCode never reads; `.opencode/` holds only agents/commands/modes/plugins/skills/tools/themes), and for Kilo Code it is `kilo.json` at the **project root** under the top-level `mcp` key. Both are **shared** user configs (`mcp.merge: true`): deep-merge the generated MCP settings while preserving user keys. **OpenCode validates each MCP entry with a strict schema** — emit only documented keys: `{ type: "remote", url, enabled, headers? }` (or `{ type: "local", command: [...], enabled, environment? }`); any extra key such as `description` or `connection_id` makes OpenCode reject the whole config and the servers silently never load. After writing it, **recommend that the user restart the AI client** — see *Recommend a restart after MCP changes* below.
...
8. **Scaffold OpenSpec.** Copy `openspec/` into the project in skip-if-exists mode. Rewrite source paths in newly copied Markdown using the shared canonical active-tool layout. On update, an existing Markdown scaffold may receive the same path migration only when its full text still exactly matches the source template; otherwise preserve it as user-authored content without opening it for transformation.
...
`AGENTS.md` placement uses the same deterministic path rewriter as all other installed text artefacts, with one difference: it selects the canonical active-tool layout per section because the file is shared by every active tool.
...
2. Resolve the **canonical artefact layout** per section. For each of `rules`, `agents`, `commands`, `skills`, walk the priority order `cursor → claude-code → kilocode → opencode → codex → other` and pick the first active tool whose adapter declares `<section>.copyTo`; use the same priority to select `mcp.target`. The result is, per file section, a `(directory, extension)` pair derived by stripping `{name}...` from the `copyTo` template. Record the canonical rules directory in `.ai-rules.json` for diagnostics and update logic; the other sections are recomputed on every refresh from the active tool set.
3. Rewrite the source text by substituting `content/<section>/...` paths with the per-section canonical installed paths so every path in the file resolves to an existing project-local file:
   ...
   - `content/skills/<rest>` → `<skillsDir>/<rest>` — skill subpaths are preserved, so anything after `content/skills/` (`SKILL.md`, `docs/<file>.md`, `tools/...`) stays unchanged.
   - The name regex matches real names, wildcards, and prose placeholders like `<name>`, so illustrative paths in the body are also rewritten consistently.
   - `content/mcp-servers.json` → the canonical active tool's `mcp.target`.
...
- **Injecting unknown keys into an OpenCode MCP entry (`description`, `connection_id`, …).** OpenCode validates each entry with a strict schema; an extra key makes it reject the whole config and load no servers. Emit only the documented keys: `{ type: "remote", url, enabled, headers? }` / `{ type: "local", command, enabled, environment? }`.
- **Emitting a Claude Code HTTP MCP entry without `"type": "http"`.** Claude Code's `.mcp.json` requires an explicit `"type": "http"` on every remote (URL-based) server; without it the current Claude Code (VS Code extension and CLI) silently does not load the server and it never appears in the tool list. Emit `{ "type": "http", "url": ... }` (plus `headers` when present), and do **not** copy the Cursor-only `connection_id` / `description` keys into the Claude Code config.
- **Cherry-picking which skills to copy.** Symptom: some skills (often the non-1C-domain ones — `caveman`, `prompt-enhancer`, `handoff`) missing from the tool's skills directory because the agent decided they were "not 1C". The protocol copies **every** folder under `content/skills/` in one directory copy; there is no per-skill relevance judgement at install time. Only Markdown path references are rewritten afterward; non-Markdown assets stay verbatim.
- **Writing Kilo MCP into the legacy `.kilocode/mcp.json` with the `mcpServers` dictionary.** Current Kilo CLI / Kilo Code (v7.x+) does not read that file — MCP must go into project-root `kilo.json` (per `adapters/kilocode.yaml > mcp.target`) under the top-level `mcp` key with per-server entries `{ "type": "remote"|"local", "url"|"command": …, "headers": {…}, "enabled": true }` for remote servers when headers are needed (see https://kilo.ai/docs/automate/mcp/using-in-cli). Writing the legacy shape silently disables MCP discovery; `/mcps` shows an empty list and agent tool calls fail because no MCP tools are exposed. The installer deep-merges only the `mcp` key into `kilo.json`, so user-added `instructions` / `skills.paths` / `permission` keys in that file are preserved across `update`.
...
- **Rewriting paths only in root `AGENTS.md`.** Rules, agents, commands, skills, and bundled OpenSpec Markdown cross-reference one another too. Leaving their `content/...` references unchanged points the installed tool back at a source tree that is not present in the target project. Run the per-tool rewrite from step 4 on every managed text artefact.
