...
function Format-TomlInlineTable {
    param([object]$Table)
    $items = @()
    foreach ($p in $Table.PSObject.Properties) {
        $items += ((Format-TomlKey $p.Name) + ' = ' + (Format-TomlString ([string]$p.Value)))
    }
    return '{ ' + ($items -join ', ') + ' }'
}

function Format-TomlBareKey {
    param([string]$Key)
    if ($Key -match '^[A-Za-z0-9_-]+$') { return $Key }
    return (Format-TomlString $Key)
}
...
function New-McpConfig-Codex {
    ...
    $lines += ''
    foreach ($s in $Servers) {
        $lines += "[mcp_servers.$(Format-TomlBareKey $s.id)]"
        $lines += 'enabled = true'
        ...
        if ($s.cwd) { $lines += 'cwd = ' + (Format-TomlString $s.cwd) }
        if ($s.env) { $lines += 'env = ' + (Format-TomlInlineTable $s.env) }
        if ($s.startupTimeoutSec) { $lines += 'startup_timeout_sec = ' + [string]$s.startupTimeoutSec }
        if ($s.toolTimeoutSec) { $lines += 'tool_timeout_sec = ' + [string]$s.toolTimeoutSec }
        if ($s.required -eq $true) { $lines += 'required = true' }
        if ($s.enabledTools) { $lines += 'enabled_tools = ' + (Format-TomlArray $s.enabledTools) }
        if ($s.defaultToolsApprovalMode) {
            $lines += 'default_tools_approval_mode = ' + (Format-TomlString $s.defaultToolsApprovalMode)
        }
        $lines += ''
    }
    return ($lines -join "`n")
...
function Invoke-McpPhase {
    ...
    foreach ($tool in $ActiveTools) {
        ...
        # but also user instructions, skills paths, custom permissions),
...
function Invoke-Init {
    ...
    # .dev.env is placed before MCP so all project parameters are present
    # before per-tool configs and AGENTS.md are rendered.
...
function Invoke-Update {
    ...
    # .dev.env runs before MCP so project parameters are refreshed before
    # per-tool MCP configs are re-rendered.
